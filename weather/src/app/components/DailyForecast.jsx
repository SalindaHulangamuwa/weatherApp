        ...
        <div className="space-y-4">
          {data.map((day, index) => {
            ...
            return (
              <div
                key={index}
                className={`flex items-center justify-between py-2 px-4 transition-all duration-200 border-b border-white/30 ${textColor}/20`}
              >
                <div className={`w-36 font-medium ${textColor}/90`}>{dateStr}</div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  />
                  <div className={`w-32 text-sm ${textColor}/70 truncate`}>
                    {day.day.condition.text}
                  </div>
                </div>
  
                <div className="w-24 text-right space-y-1">
                  <div className={`font-semibold ${textColor}`}>{day.day.maxtemp_c}°C</div>
                  <div className={`text-sm ${textColor}/60`}>{day.day.mintemp_c}°C</div>
                </div>
              </div>
            )
          })}
